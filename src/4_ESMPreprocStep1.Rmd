# This Rmarkdown is based on the step-by-step guidelines for preprocessing ESM data provided by Revol et al., (2024) via their website: https://preprocess.esmtools.com/, and contains code from or adapted from "Step 1: Import data and preliminary preprocessing". 
...
# Create new variable called 'source' in each EMA dataset to document its origin and merge.
```{r}
part6beeped_1 <- readRDS("C:\\Users\\Walt\\OneDrive - The Pennsylvania State University\\Projects\\ASH\\DVAL\\Users\\Walter\\Preprocessing\\Interim Data\\part6beeped_1.rds") %>%
  mutate(source = "part6")
part2beeped_1 <- readRDS("C:\\Users\\Walt\\OneDrive - The Pennsylvania State University\\Projects\\ASH\\DVAL\\Users\\Walter\\Preprocessing\\Interim Data\\part2beeped_1.rds") %>%
  mutate(source = "part2")
merged_ema <- bind_rows(part6beeped_1, part2beeped_1)
```

#Read in baseline data and extract key covariates for ema (The below code extracts covariates based on analytic plan as of 6/19/25)
```{r}
baseline <- readRDS("C:\\Users\\Walt\\OneDrive - The Pennsylvania State University\\Projects\\ASH\\DVAL\\Users\\Walter\\Preprocessing\\Interim Data\\baseline2.rds")
baseline <- baseline %>%
  dplyr::select(participant_id, race, age, gender, total_household_income, bank_balance, ftnd_sum, cigsperday)
```
...
```{r}
saveRDS(d_full, "C:\\Users\\Walt\\OneDrive - The Pennsylvania State University\\Projects\\ASH\\DVAL\\Users\\Walter\\Preprocessing\\Interim Data\\EMA_and_baseline1.rds")
```
