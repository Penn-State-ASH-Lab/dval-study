...
library(ggplot2)
```
#This RMD contains code to merge baseline datasets and prepare the data for analysis. 

#There are two files that contain data collected at the baseline session. One is called 'QuestionnairesA' and the other 'QuestionnairesB'. Read both datasets into the environment:
```{r}
# Define the file paths for Excel files (.xlsx)
Apath <- "C:/Users/Walt/OneDrive - The Pennsylvania State University/Projects/ASH/DVAL/3. Organized raw data/Qualtrics/1.Baseline-session/QuestionnairesA.xlsx"
Bpath <- "C:/Users/Walt/OneDrive - The Pennsylvania State University/Projects/ASH/DVAL/3. Organized raw data/Qualtrics/1.Baseline-session/QuestionnairesB.xlsx"

# Read Excel files into data frames A and B
A <- readxl::read_excel(Apath)
B <- readxl::read_excel(Bpath)
```
...
#The baseline data are now ready to be merged with the EMA data. Before merging the baseline and EMA data though, the EMA data must be prepared. Code for pre-merge actions on EMA data can be found in the following location: DVAL/Users/Walter/Preprocessing/PremergePreprocessing.
...
```{r}
saveRDS(baseline2, "C:\\Users\\Walt\\OneDrive - The Pennsylvania State University\\Projects\\ASH\\DVAL\\Users\\Walter\\Preprocessing\\Interim Data\\baseline2.rds")
```
